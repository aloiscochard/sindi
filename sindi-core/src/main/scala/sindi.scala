...
trait Sindi[Q] /* extends InjectionTemplate[Q] */ {
  def autowire[A, B](f: (A) => B)(implicit binding: Binding[A, Q]): B = f(inject[A])
  def autowire[A, B](f: (Option[A]) => B)(implicit option: Option[Binding[A, Q]] = None): B = f(injectOption[A])
  def autowire[T0, T1, B](f: (Either[T0, T1]) => B)
    (implicit either: Either[Binding[T0, Q], Binding[T1, Q]]): B = f(injectEither[T0, T1])
...
class QualifiersOps[Q0, Q1](tuple: (Qualifier[Q0], Qualifier[Q1])) { 
  ...
  def injectOption[T](implicit o0: Option[Binding[T, Q0]] = None, o1: Option[Binding[T, Q1]] = None) =
    o0.orElse(o1).map(_.inject)
}
